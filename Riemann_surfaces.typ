...
#definition[Divisor Group][
  ...
  denoted by $ op("Div") lr((X)) = ZZ^(plus.circle X)$. An element of $op("Div") lr((X))$ is called a #strong[divisor] on $X$. A divisor $D$ on $X$ can
...
#definition[
  ...
  Let $X$ be a Riemann surface and $f$ be a nonzero meromorphic function on $X$. We have the following abelian group homomorphism:
  $
  op("div") : cal(M) lr((X))^* & --> op("Div") lr((X))\
  f & arrow.long.bar sum_(x in X) "ord"_x lr((f)) x .
  $
  The #strong[divisor of $f$] defined as $op("div") (f)$. Any divisor of this form is called a #strong[principal divisor] on $X$. The group of principal divisors on $X$ is denoted by $op("PDiv") lr((X))= op("div")lr((cal(M) lr((X))^*))$.
]
...
#proof[
  $op("div")(f)=0$ implies that $f$ is holomorphic on $X$. Since $X$ is compact, $f$ is a constant function. The converse is trivial.
]
...
$)
  ...
  For compact Riemann surfaces $X$, the #strong[restricted Picard group] of $X$ is
...
#canvas(
  length: 0.72cm,
  {
    ...
    let angle_colors = range(angle_n).map(i=> color.map.rainbow.at(int(i / angle_n * color.map.rainbow.len())).darken(20%))
